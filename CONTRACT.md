...
| `POST` | `/missions/{id}/stop` | `{action?:"stop"\|"pause"\|"resume", approval_required?:bool, reason?}` (≥1 of action/approval_required) | **loop-control endpoint** — writes `runs/<id>/control.json` (§9.2). `stop` ⇒ the loop emits `mission_finished{reason:"user_stop"}`; if no loop is running, the API emits it directly. **`action` is optional**: omit it to set `approval_required` WITHOUT changing run_state (arming/disarming the gate never un-pauses a paused mission). | `{action, run_state}` |
